...
import yaml
import pandas as pd
from map_rna_atac import main as scmap

import sys
sys.path.append("M2Mmodel")
from utils import setup_seed, PairDataset
...
def pair_data_process(rna, 
    ...
    dataset = PairDataset(rna, atac, enc_max_len, dec_max_len, rna_num_bin, multiple, atac2rna)
...
def main():
    ...
    parser.add_argument('-c', '--config', type = str, default = None, help = "Config of this process. If not set, will automatically use 'atac2rna_config.yaml' or 'rna2atac_config.yaml'") 
    ...
    parser.add_argument('--manually', action = 'store_true', help = 'By default the script will map atac and rna to vocab of cisFormer and divide them into train and test set. If you set the parameter, this script will not divide the input dataset.')
    ...
    if args.config:
        ...
        print("Config file not specified, defaulting to 'atac2rna_config.yaml'")
        config = "atac2rna_config.yaml" if atac2rna else "rna2atac_config.yaml"
    ...
    if not manually:
        mapped_dict = scmap(rna_dir, atac_dir, save_dir)
        ...
        pair_data_process(train_rna, 
                          ...
                          shuffle = True, 
                          save_dir = os.path.join(save_dir, "cisformer_train_dataset"),
                          cnt= args.cnt,
                          **pair_args)
        if atac2rna:
            pair_data_process(test_rna, 
                              ...
                              dec_max_len = config["datapreprocess"]['dec_max_len'], 
                              shuffle = False, 
                              save_dir = os.path.join(save_dir, "cisformer_test_dataset"),
                              cnt= float('inf'),
                              **pair_args)
        else:
            pair_data_process(test_rna, 
        ...
        mapped_dict = scmap(rna_dir, atac_dir, save_dir, divide=False)
        rna = mapped_dict['rna_new']
        atac = mapped_dict['atac_new']
        pair_data_process(rna, 
                          atac, 
                          dec_max_len = config["datapreprocess"]['dec_max_len'] if not args.dec_whole_length else "whole", 
