...
from M2Mmodel.block import TransformerBlock, default, route_args
...
def whole_attn_score_matrix_generator(q, k, q_mask = None, k_mask = None):
    # q,k shape: b h n d
    # mask shape: b n
    b, _, n1, _ = q.shape
    _, _, n2, _ = k.shape
...
def whole_attn_score_matrix(q, k, q_mask = None, k_mask = None):
    """_summary_
    ...
    try:
        with autocast():
            output = whole_attn_score_matrix_generator(q, k, q_mask = q_mask, k_mask = k_mask)
    except RuntimeError as e:
        if "CUDA out of memory" in str(e):
            timeout_seconds = 3
            user_input = input_with_timeout("CUDA out of memory, using CPU to compute (y/n)?", timeout_seconds)
            if user_input is None:
                print("Input timed out. Automatically choosing 'n'.")
                continue_run = 'n'
            else:
                continue_run = user_input.lower()
            if continue_run.lower() in ["y", "yes"]:
                q = q.float().cpu()
                k = k.float().cpu()
                if exists(q_mask):
                    q_mask = q_mask.cpu()
